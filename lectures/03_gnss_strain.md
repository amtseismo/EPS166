# From Interseismic Velocity to Tectonic Strain
...
- one rigid-body rotation rate: $\dot\omega$
...
| Site | East coordinate (km) | North coordinate (km) | East velocity (mm/yr) | North velocity (mm/yr) |
|---|---:|---:|---:|---:|
| 1 | 518 | 243 | -1.0 | 1.2 |
| 2 | 523 | 244 | -1.0 | 0.8 |
| 3 | 520 | 248 | 0.4 | 0.2 |

We will use kilometers for position and millimeters per year for velocity. Therefore, the velocity gradients initially have units of $(\mathrm{mm/yr})/\mathrm{km}$.
...
$$
\bar{x}=520.333\ \mathrm{km},
\qquad
\bar{y}=245.000\ \mathrm{km}.
$$
...
| Site | $X=x-\bar{x}$ (km) | $Y=y-\bar{y}$ (km) |
|---|---:|---:|
| 1 | -2.333 | -2.000 |
| 2 | 2.667 | -1.000 |
| 3 | -0.333 | 3.000 |
...
$$
\bar{v}_E=-0.533\ \mathrm{mm/yr},
\qquad
\bar{v}_N=0.733\ \mathrm{mm/yr}.
$$
...
| Site | $v_E'$ (mm/yr) | $v_N'$ (mm/yr) |
|---|---:|---:|
| 1 | -0.467 | 0.467 |
| 2 | -0.467 | 0.067 |
| 3 | 0.933 | -0.533 |

The relative velocities sum to zero, so they change the triangle without moving its centroid.
...
## Describe the Change with a Velocity Gradient

The graphical construction gives us an original position $(X_i,Y_i)$ and a relative velocity $(v_{E,i}',v_{N,i}')$ at every vertex. We seek the linear transformation
...
The four unknowns are the spatial derivatives

$$
a=\frac{\partial v_E}{\partial x},\quad
b=\frac{\partial v_E}{\partial y},\quad
c=\frac{\partial v_N}{\partial x},\quad
d=\frac{\partial v_N}{\partial y}.
$$

---

## Write the Triangle as an Inverse Problem

Stack the two velocity components from all three stations:
...
Because $1\ (\mathrm{mm/yr})/\mathrm{km}=1000\times10^{-9}\ \mathrm{yr}^{-1}$, multiply the matrix by 1000 to express its entries in nanostrain or nanoradians per year.
...
## Separate Shape Change from Rotation

The symmetric part of the velocity gradient changes the triangle's shape:
...
The antisymmetric part rotates the triangle without changing its shape:
...
For this triangle, the eigenvalues of $\dot{\boldsymbol\epsilon}$ are

$$
\dot\epsilon_1=22.2\ \mathrm{nstrain/yr},
\qquad
\dot\epsilon_2=-265.7\ \mathrm{nstrain/yr}.
$$

The maximum principal axis is oriented $32.5^\circ$ counterclockwise from east; the minimum principal axis is perpendicular to it.
...
For this example, $\dot\omega=-173.9$ nrad/yr: the triangle rotates clockwise while it changes shape.
...
## Convert the Principal Rates to Mapped Quantities

The principal rates give the quantities shown in panels (a)--(c) of Kreemer and Young's strain-rate map:
...
> **Interpretation:** This triangle undergoes strong net contraction, modest perpendicular extension, and clockwise rotation.
...
1. estimate the local velocity-gradient tensor $\nabla\mathbf v$
